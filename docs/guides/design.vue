<template>
  <div v-if="scssVars.length>0">
    ...
    <h2>响应式体系</h2>
    <div>虽然从用户体验与业界主流应用来说，并不推荐同时响应PC,Pad,phone三端的设计，但响应式布局仍然有其不可取代的优点，所以天闻UI框架体然保留了类bootstrap的超强响应式体系，但为了更自然的书写css，以桌面应用为主的系统(pc-web)端摒弃了Bootstrap的移动端优先原则，而改为了PC端优先，以移动端为主的系统，依然保持移动端优先。</div>
    <table>
      <thead>
        <tr>
          <th style="width:10%;">分类</th>
